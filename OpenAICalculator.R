...
extract_openai_content <- function(response) {
  if (!is.null(response$choices) && length(response$choices) >= 1) {
    ...
    if (!is.null(first_choice$message)) {
      message_obj <- first_choice$message
      if (!is.null(message_obj$content)) {
        content <- message_obj$content
        if (is.character(content) && length(content) >= 1) return(content[[1]])
...
ask_openai_assistant <- function(prompt, top_matches, model = "gpt-5.4") {
  api_key <- Sys.getenv("OPENAI_API_KEY", unset = "")
  if (!nzchar(api_key)) {
    return(
      paste(
        "OpenAI response unavailable: set OPENAI_API_KEY.",
        "In RStudio run usethis::edit_r_environ(), add OPENAI_API_KEY=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\", then restart RStudio."
      )
    )
  }
  
  match_lines <- top_matches %>%
    mutate(summary = paste0(photo_id, " | ", site_type, " | similarity=", round(similarity, 1), "%")) %>%
    pull(summary)
  ...
  "OpenAI response unavailable: empty response received."
}
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    tags$small(
      "RStudio setup: install.packages('openai') and set OPENAI_API_KEY in .Renviron (use usethis::edit_r_environ())."
    ),
    fileInput("uploaded_photo", "Upload reference photo", accept = c(".png", ".jpg", ".jpeg")),
    textInput("openai_model", "OpenAI model", value = "gpt-5.4"),
...
server <- function(input, output, session) {
  ...
  ai_message <- reactiveVal("Upload an image and click compare to get OpenAI guidance.")
  ...
  observeEvent(input$compare_photo, {
    ...
    ai_message(
      tryCatch(
        ...
        error = function(e) paste("OpenAI response unavailable:", e$message)
      )
    )
  })
  ...
  output$ai_response <- renderUI({
    ...
    tags$div(
      ...
      tags$p(style = "background:#ecfeff;padding:10px;border-radius:8px;", ai_message())
    )
  })
