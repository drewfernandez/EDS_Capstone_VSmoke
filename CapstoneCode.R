...
make_ellipse <- function(lon, lat, a_km, b_km, angle_deg, shift_frac = 0.5, n = 60) {
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    # Location inputs
    h4("📍 Burn Location", style = "color: #2E8B57;"),
    ...
    # Burn parameters
    h4("🔥 Burn Parameters", style = "color: #2E8B57;"),
...
server <- function(input, output, session) {
  ...
  compass_to_degrees <- function(direction) {
    switch(direction,
           "North (N)" = 0,
           "Northeast (NE)" = 45,
           "East (E)" = 90,
  ...
  observeEvent(input$predict, {
    ...
    # Generate prediction
    wind_deg <- compass_to_degrees(input$wind_direction) + 180
  ...
  output$smoke_map <- renderLeaflet({
    ...
    ellipse_data <- values$prediction_data %>%
      group_by(aqi_bin) %>%
      summarise(max_dist = max(x_rot, na.rm = TRUE)) |> 
    ...
    m %>%
      ...
      addCircles(
        lng = values$burn_lon,
        lat = values$burn_lat,
        radius = sqrt(input$acres * 4046.86 / pi),  # acres → m² → radius (m)
        color = "orange",
        weight = 2,
        fill = FALSE,
        popup = "Burn Area"
